...
import type { CompareStatsProps } from '../models/CompareStats.model'
...
export function CompareStats({
  ...
  if (stats.length === 0) {
    ...
    return (
      <StatusNote
        error={error}
        message={error ? "Couldn't load the comparison" : 'Loading…'}
        className={cn(
    ...
    return (
      <div className="glass mx-[18px] mb-4 flex flex-col gap-px rounded-2xl px-[18px] pt-2 pb-1">
        {stats.map((stat) => (
          ...
          >
            <span className="text-chalk-ghost w-[74px] text-[9.5px] tracking-[0.16em]">
              {stat.label}
            </span>
