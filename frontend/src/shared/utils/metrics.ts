...
import type { Alarm, EnvMetric, Metric, MetricSpec, Room, Severity } from '@/shared/models/types'
...
/**
 * Well-known room ids, mirroring `hermes/domain/catalog.py`. Use these
 * instead of a bare string literal for the fallback defaults shown before
 * `/api/meta` resolves — the authoritative room list always comes from the
 * API, so a room added to the backend catalog needs no change here to show up.
 */
export const KITCHEN: Room = 'kitchen'
export const BEDROOM: Room = 'bedroom'
export const OUTSIDE: Room = 'outside'
...
/** The empty state for an alarm list, shared by its desktop and phone variants. */
export function alarmEmptyMessage(loading: boolean | undefined, error: Error | undefined): string {
  if (error) return "Couldn't load. Check the connection and try again."
  if (loading) return 'Loading…'
  return 'No alarms in this window.'
}

/** Room + the chip that actually produced the alarm, e.g. "KITCHEN · MQ-2". */
export function alarmSource(alarm: Alarm): string {
  const sensor = sensorForMetric(alarm.sensor, alarm.metric)
  return [roomLabel(alarm.room), sensor].filter(Boolean).join(' · ').toUpperCase()
}

export function alarmSeverityLabel(alarm: Alarm): string {
