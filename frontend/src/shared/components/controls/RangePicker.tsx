...
import { formatDayShort } from '@/shared/utils/format'
import { withAlpha } from '@/shared/utils/metrics'
import { cn } from '@/shared/utils/cn'
import type { MonthGrid } from '@/shared/models/dates.model'
import type { RangePickerProps } from '../../models/controls/RangePicker.model'
...
export function RangePicker({ range, onChange, accent, variant = 'popover' }: RangePickerProps) {
  ...
  const [anchor, setAnchor] = useState(
    () => new Date(range.to.getFullYear(), range.to.getMonth() - 1, 1),
  )
  ...
  const trigger = (
    <button
      ...
      className={cn(
        ...
        variant === 'sheet'
          ? 'w-full px-3.5 py-3.5 text-[11.5px]'
          : 'px-3.5 py-[7px] text-[11.5px]',
      )}
        ...
        {(open || closing) && (
          <div className="fixed inset-0 z-30 flex flex-col justify-end">
            ...
            >
              ...
              {monthNav}
              {calendar(38, 12.5)}
              <div className="text-chalk-ghost mt-3.5 mb-2.5 text-[10px] tracking-[0.14em]">
                {hint}
              </div>
        ...
        createPortal(
          ...
          >
            <div className="border-white/10 flex min-w-[152px] flex-col gap-0.5 border-r px-3 py-4">
              <span className="text-chalk-ghost px-3 pb-2 text-[9.5px] tracking-[0.16em]">
                PRESETS
              </span>
              {RANGE_PRESETS.map((preset) => (
                <button
