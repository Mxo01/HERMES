...
import { formatDayShort, formatMetric } from '@/shared/utils/format'
import { withAlpha } from '@/shared/utils/metrics'
import type { Focus, HeatmapProps } from '../../models/charts/Heatmap.model'
  ...
  return (
    <div className="relative" ref={container}>
      <div className="flex gap-2.5">
        ...
        <div className="min-w-0 flex-1" onPointerLeave={() => setFocus(null)}>
          <div className="mb-1.5 grid grid-cols-24 gap-1">
            {hours.map((hour, index) => (
              <div
                key={index}
                className="text-chalk-faint text-center text-[9px] tracking-[0.04em]"
              >
                {hour}
              </div>
            ))}
